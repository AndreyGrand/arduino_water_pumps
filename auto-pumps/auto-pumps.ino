...
void setup() {
  ...
  if (!digitalRead(SW)) {          // если нажат энкодер, сбросить настройки до 1
    Serial.println("нажат энкодер, сбросить настройки до 1");
    lcd.setCursor(0, 0);
...
void buttonCheck(){
  bool btn_pressed = digitalRead(PUMP_BTN) == LOW; 
   ...
   Serial.println(pump_pins[current_pump]);
	}
	else if (!btn_pressed && PUMP_BTN_CLICKED){
		PUMP_BTN_CLICKED = false;
		digitalWrite(pump_pins[current_pump], !SWITCH_LEVEL);
...
void encoderTick() {
  ...
  if (enc1.isTurn()) {                               // если был совершён поворот
    displayOn();
    //Serial.println("был повёрнут " + current_set);
    //Serial.println("поворот на " + enc1.normCount);
